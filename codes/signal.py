...
class MotSignal(ABCSignal):
    def __init__(self, dataset_name, conf, uvadmode='partial'):
        ...
        if conf.cleanse_scorename:
            cleanse_scorepath = os.path.join(
            base_dir,
            "features",
            dataset_name,
            "cleansescores",
            f"{uvadmode}_{conf.cleanse_scorename}_flat.npy"
            )

            # print("Cleanse score path: ", cleanse_scorepath)
            cleanse_score = np.load(cleanse_scorepath, allow_pickle=True)
            # print("Cleanse score shape", cleanse_score.shape)
            thres = np.percentile(cleanse_score, conf.percentile_cleanse)
            print("conf.percentile_cleanse", conf.percentile_cleanse)
