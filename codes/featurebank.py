...
def _get_mergemode(dataset_name, feature_name, mode):
    dpath = os.path.join(base_dir, "features", dataset_name)
    if mode == 'train':
        ...
        # print("Train features shape", d1.shape)
        # print("Test features shape", d2.shape)
        d1 = np.concatenate(d1, 0)
        d2 = [v for v in d2 if len(v)]
        d2 = np.concatenate(d2, 0)
        # print("Merged dataset shape ", np.concatenate([d1, d2]).shape )
...
def _get_partialmode(dataset_name, feature_name, mode):
    ...
    if mode == 'train':
        ...
        d1 = np.load(fpath1, allow_pickle=True)
        # print("Test features shape", d1.shape)        
        d1 = np.concatenate(d1, 0)
        return d1
